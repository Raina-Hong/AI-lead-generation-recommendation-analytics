...
The project uses the public Olist Brazilian e-commerce dataset and creates additional analytical tables for user funnel simulation, intent classification, lead scoring, recommendation strategy design, and simulated A/B test evaluation.
...
This table contains simulated user interaction events used for funnel analysis. The original Olist dataset does not contain real clickstream logs, so these events are generated using reproducible business assumptions.
...
| `event_type` | Simulated event type: view, click, add_to_cart, inquiry, or purchase |
| `order_id` | Related order identifier |
| `product_id` | Related product identifier |
| `seller_id` | Related seller identifier |
| `category` | Product category |
| `device_type` | Simulated device type, such as Web, iOS, or Android |
| `traffic_source` | Simulated traffic source, such as For You Feed, Live Stream, Search, Shop Tab, or Creator Profile |
| `price` | Product price |
...
This table contains rule-based, LLM-inspired customer intent classification and lead scoring outputs.
...
| `traffic_source` | Simulated traffic source |
| `device_type` | Simulated device type |
| `user_value_segment` | User value segment |
| `total_events` | Number of simulated events linked to the order or user journey |
| `viewed` | 1 if the user had a simulated view event; otherwise 0 |
| `clicked` | 1 if the user had a simulated click event; otherwise 0 |
| `added_to_cart` | 1 if the user had a simulated add-to-cart event; otherwise 0 |
| `inquired` | 1 if the user had a simulated inquiry event; otherwise 0 |
...
| `experiment_group` | Group assignment used for simulated experiment design |
| `recommendation_date` | Recommendation date |

### `data/final/fact_recommendation_experiment.csv`

This table contains simulated A/B test outcomes for the recommendation experiment.

| Field | Description |
|---|---|
| `click_probability` | Simulated probability that the user clicks the recommendation |
| `inquiry_probability` | Simulated probability that the user makes an inquiry |
| `purchase_probability` | Simulated probability that the user purchases |
| `clicked` | Simulated click outcome: 1 if clicked, otherwise 0 |
| `inquired` | Simulated inquiry outcome: 1 if inquired, otherwise 0 |
| `purchased` | Simulated purchase outcome: 1 if purchased, otherwise 0 |
| `revenue` | Simulated revenue from the recommendation outcome |
...
| Output Table | Description |
|---|---|
| `funnel_summary.csv` | Overall simulated funnel by event stage |
| `daily_funnel_trend.csv` | Daily simulated funnel trend |
...
| `traffic_source_funnel.csv` | Funnel performance by simulated traffic source |
...
| Output Table | Description |
|---|---|
| `ab_test_summary.csv` | Simulated A/B test summary by experiment group |
...
| `traffic_source` | Simulated acquisition or engagement source |
| `device_type` | Simulated user device type |
| `event_type` | Simulated user journey event type |
...
| `experiment_group` | Control or treatment group in the simulated A/B test |
| `clicked` | Simulated click outcome |
| `inquired` | Simulated inquiry outcome |
| `purchased` | Simulated purchase outcome |
| `revenue` | Simulated revenue generated in the A/B test |

---

## 6. Notes on Simulated Fields

Several fields in this project are simulated because the original Olist dataset does not include real platform clickstream or experiment data.

Simulated fields include:
...
The simulated fields are used to demonstrate analytics design, lead generation logic, recommendation strategy evaluation, and dashboard storytelling. They should not be interpreted as real user behaviour collected from the Olist platform.
